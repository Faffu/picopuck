...
> Unofficial project, not affiliated with or endorsed by Valve or Nintendo.
> Independent from [OpenPuck](https://github.com/ripps818/openpuck), which
> targets the nRF52840.

Fork of [OGX-Mini](https://github.com/wiredopposite/OGX-Mini) that bridges a **Steam Controller 2** over Bluetooth to USB on a
**Pico 2 W**: Nintendo Switch Pro Controller by default, with gyro and rumble,
or any OGX-Mini mode such as XInput.

* Architecture, button mapping, build, flash, pairing, modes and report capture:
  [docs/PICO_BRIDGE.md](docs/PICO_BRIDGE.md)
* What works and what is missing: [docs/ROADMAP.md](docs/ROADMAP.md)
* Hardware checks and results: [docs/TESTING.md](docs/TESTING.md)
* Prebuilt firmware: `artifacts/bridge/` (gamepad), `artifacts/capture/` (report capture)

This fork drops upstream's `Firmware/ESP32_Blueretro` (the vendored ESP-IDF for
the Pico + ESP32 BlueRetro board), which the Pico 2 W build doesn't use; get it
from upstream OGX-Mini if you need that board.

Mode combos differ from upstream in this fork: Switch Pro (the default) is
Start + RB + D-pad Down, report capture is Start + LB + D-pad Down.

The upstream OGX-Mini README follows.

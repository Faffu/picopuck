...
namespace ButtonCombo {
    ...
    static constexpr uint32_t SWITCH_PRO = BUTTON_COMBO(Gamepad::BUTTON_START | Gamepad::BUTTON_RB, Gamepad::DPAD_DOWN);
    static constexpr uint32_t CAPTURE   = BUTTON_COMBO(Gamepad::BUTTON_START | Gamepad::BUTTON_LB, Gamepad::DPAD_DOWN);
...
#else // MAX_GAMEPADS == 1
    DeviceDriverType::SWITCH_PRO, //fork default
    DeviceDriverType::CAPTURE,
    DeviceDriverType::XBOXOG, 
    DeviceDriverType::XBOXOG_SB, 
    DeviceDriverType::DINPUT, 
    DeviceDriverType::SWITCH, 
...
static constexpr std::array<ComboMap, 11> BUTTON_COMBO_MAP = {{
    { ButtonCombo::SWITCH_PRO, DeviceDriverType::SWITCH_PRO },
    { ButtonCombo::CAPTURE,    DeviceDriverType::CAPTURE    },
...
bool UserSettings::check_for_driver_change(Gamepad& gamepad)
{
    ...
    //The capture build can't leave capture mode, a combo would only reboot it
    //and drop the serial port mid capture.
    (void)gamepad;
    return false;
#endif
...
{
#if defined(CONFIG_OGXM_FORCE_CAPTURE)
    //Capture build: the mode combos need a working decoder, which is what the
    //capture is for, so boot straight into capture whatever flash says.
    return DeviceDriverType::CAPTURE;
#endif

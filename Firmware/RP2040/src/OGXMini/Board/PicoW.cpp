...
void core1_task() {
    //NVS writes on core0 pause this core, flash is unreadable meanwhile.
...
void pico_w::run() {
    //BTstack on core1 saves Bluetooth bonds with flash_safe_execute, which
    //refuses to write unless this core can be paused. Without this, bonds only
    //lived in RAM and every reboot needed a new pairing.
    multicore_lockout_victim_init();

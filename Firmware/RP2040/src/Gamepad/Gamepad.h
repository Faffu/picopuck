...
public:
    ...
    static constexpr uint8_t MOTION_VALID_ACCEL    = 0x01;
    static constexpr uint8_t MOTION_VALID_GYRO     = 0x02;
    static constexpr uint8_t MOTION_VALID_PAD_L    = 0x04;
    static constexpr uint8_t MOTION_VALID_PAD_R    = 0x08;
    static constexpr uint8_t MOTION_VALID_BACK     = 0x10;

    static constexpr uint8_t BACK_BUTTON_L4 = 0x01;
    static constexpr uint8_t BACK_BUTTON_R4 = 0x02;
    static constexpr uint8_t BACK_BUTTON_L5 = 0x04;
    static constexpr uint8_t BACK_BUTTON_R5 = 0x08;
...
#pragma pack(push, 1)
    ...
    //Motion, trackpad and back buttons, filled in by inputs that provide them.
    //Units: accel in milli-g, gyro in 0.1 deg/s, trackpads centered on 0.
    struct PadMotion
    {
        int16_t accel_x;
        int16_t accel_y;
        ...
        int16_t trackpad_rx;
        int16_t trackpad_ry;
        uint8_t back_buttons;
        uint8_t valid; //MOTION_VALID_* bits

        PadMotion()
        {
            std::memset(this, 0, sizeof(PadMotion));
        }
    };
...
#pragma pack(pop)
    ...
    inline PadMotion get_pad_motion()
    {
        mutex_enter_blocking(&pad_motion_mutex_);
        PadMotion pad_motion = pad_motion_;
        mutex_exit(&pad_motion_mutex_);

        return pad_motion;
    }

    inline void set_pad_motion(const PadMotion& pad_motion)
    {
        mutex_enter_blocking(&pad_motion_mutex_);
        pad_motion_ = pad_motion;
        mutex_exit(&pad_motion_mutex_);
    }

    inline void reset_pad_motion()
    {
        mutex_enter_blocking(&pad_motion_mutex_);
        pad_motion_ = PadMotion();
        mutex_exit(&pad_motion_mutex_);
    }
...
private:    
    ...
    mutex_t pad_motion_mutex_;

    PadOut pad_out_;
    PadIn pad_in_;
    ChatpadIn chatpad_in_{0};
    PadMotion pad_motion_;
